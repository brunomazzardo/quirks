#!/usr/bin/env bash
# Stop the shape companion server and clean up.
# Adapted from Superpowers' visual-companion stop script (MIT).
# Usage: stop-server.sh <session_dir>
#
# Kills the server process. Only deletes session directory if it's
# under /tmp (ephemeral). Persistent directories (.quirks/shape-sessions/)
...
command_has_server_id() {
  local pid="$1"
  local expected="$2"
  local expected_arg="--shape-server-id=$expected"
  if [[ -r "/proc/$pid/cmdline" ]]; then
    local arg
