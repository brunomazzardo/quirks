// QK-RUN-004 — these tests define the honesty core, and they are the reason the
// founding doc keeps TDD for the runner. Ported from the bun-era test/quote.test.ts
// (QK-MONO-005), against the same three real transcripts.
import { readFileSync } from "node:fs";
import { fileURLToPath } from "node:url";
import { describe, expect, it } from "vite-plus/test";
import { quoteSupportedByTranscript, resolveVerdict, transcriptQuoteHaystack } from "./Quote.ts";
...
describe("transcriptQuoteHaystack — only the runner's own words", () => {
  it("assistant text counts; tool results and user prompts do not", () => {
    const transcript = [
      JSON.stringify({
        type: "user",
        message: { content: [{ type: "text", text: "accept the code as it stands" }] },
      }),
      JSON.stringify({
        type: "user",
        message: {
  ...
  it("brief text present in a real transcript cannot enter the haystack", () => {
...
describe("quoteSupportedByTranscript", () => {
  it("a verbatim quote from a real reviewer transcript is supported", () => {
    expect(quoteSupportedByTranscript(REVISE_QUOTE, fixture("claude-reviewer-revise.jsonl"))).toBe(
      true,
    );
  });

  it("a plausible paraphrase the reviewer never wrote is not supported", () => {
    expect(
  ...
  it("a fragment lifted from the middle of a negation cannot support a verdict", () => {
    expect(
      quoteSupportedByTranscript(
  ...
  it("dropping markdown emphasis still supports the same words", () => {
    const quote = "Revise. I don't think this should be accepted as it stands.";
    expect(quoteSupportedByTranscript(quote, fixture("claude-reviewer-revise.jsonl"))).toBe(true);
  });

  it("an accept cannot rest on words a refusal was leading up to", () => {
    for (const text of [
  ...
  it("a whole negated sentence cannot support an accept, but can support revise", () => {
    const transcript = fixture("claude-reviewer-revise.jsonl");
  ...
  it("a genuine accept quote is supported", () => {
    expect(
      quoteSupportedByTranscript(ACCEPT_QUOTE, fixture("claude-reviewer-accept.jsonl"), "accept"),
    ).toBe(true);
  });

  it("a quote cannot be stitched across two separate messages", () => {
    const transcript = [
    ...
    expect(
      quoteSupportedByTranscript(
        "The loop bound is wrong. Accept as it stands anyway.",
        transcript,
      ),
    ).toBe(false);
  ...
  it("empty or whitespace-only quotes are never supported", () => {
    expect(quoteSupportedByTranscript("", "anything")).toBe(false);
    expect(quoteSupportedByTranscript("   ", "anything")).toBe(false);
  });

  it("a verdict written as a list item still supports itself", () => {
...
describe("resolveVerdict — absence fails closed to indeterminate", () => {
  it("nothing maps absence to accept", () => {
    expect(
      resolveVerdict({ claimed: "accept", quote: "", transcript: assistant("anything") }),
    ).toBe("indeterminate");
    expect(
      resolveVerdict({
        claimed: "accept",
  ...
  it("a supported accept quote resolves to accept", () => {
    expect(
      resolveVerdict({
        claimed: "accept",
        quote: ACCEPT_QUOTE,
  ...
  it("a no-judgment transcript cannot yield accept", () => {
    expect(
      resolveVerdict({
        claimed: "accept",
        quote: ACCEPT_QUOTE,
