...
// Ported verbatim from the bun-era src/run/failure-policy.ts (QK-MONO-005).
...
export function decideFailure(input: {
  ...
  if (input.landingCommit) {
    return {
      action: "hold",
      blockDependents,
      reason: `park-on-issue: verified landing ${input.landingCommit} exists for ${input.failedId}${detail} — holding for the operator, never unassigning verified work`,
    };
  }
  return {
    action: "release",
