...
//!
//! Quirks workbench overlay of the SDK's generated TS-core wiring.
//! Adds `Options.web_panes` so the preview webview snaps to the
//! `preview-pane` markup anchor (QK-NAT-002). `build.zig` copies this
//! file over the SDK entry before staging so ejected `addApp` picks it up.
...
pub fn main(init: std.process.Init) !void {
    ...
    // Preview webview: snap to the empty panel labelled "preview-pane" and
    // navigate from model.previewUrl / model.reloadToken (QK-NAT-002/003).
    if (comptime @hasField(Model, "previewUrl") and @hasField(Model, "reloadToken")) {
        options.web_panes = previewPanes;
    }
