...
## 2026-07-28 — Post-reversal audit (`QK-MONO`, `QK-WB`)

Two adversarial review passes over `342d278..1daec9e` — the whole t3code reversal. What they
found that changed a decision, rather than merely a line:

| | Decision | Where |
|---|---|---|
| S23 | **The daemon refuses cross-origin writes.** "Loopback bind, no CORS headers" was treated as a boundary in three separate comments; it is not one. `request.json` parses a body whatever the content type says, so a POST sent as `text/plain` is a CORS *simple* request the browser sends with no preflight, and the port is `45000 + sha256(root) % 15000` — 15,000 wide, sprayable in seconds. The response is discarded for want of CORS, which settles reads and settles nothing about side effects: `POST /v1/pty/sessions` spawns a shell from caller-supplied argv, so any page in any tab could run a command. **This is not the auth the founding document defers** (no identity, no token, no capability, nothing to configure) — it is the loopback assumption the rest of the design already rests on, made true. | `http/Wire.ts` |
| S23a | **The boundary is LOOPBACK, and "unsafe" is not the same question as "unsafe verb".** Two corrections the review pass found in S23's own first cut. (1) `GET …/socket` upgrades to a WebSocket whose `input`/`resize` frames reach the shell, and WebSockets obey neither the same-origin policy nor preflight — keying off the verb left the exec surface open through the one door that does not look like a write. (2) Comparing `Origin` against `Host` looks tighter and is both weaker and wrong: `Host` is client-supplied and any proxy rewrites it, so apps/web's `changeOrigin: true` dev proxy would have made every write in `pnpm dev` a 403. A page cannot give itself a loopback origin; what this refuses is a page on the open internet. | `http/Wire.ts` |
| S24a | **The reviewer's role is in its PROMPT, not only in its brief.** Same class as S23a: the brief gained `review.instructions`, but every adapter's prompt stayed role-blind — claude got a bare path, and cursor's said "Read the brief at `<path>` and **complete it**", an instruction to implement handed to a reviewer. The contract reached the runner only if it happened to read a nested brief field. `reviewPromptText` states the role and the two declaration lines; the brief keeps them as the durable record. Referenced by path, never inlined — argv is world-visible in `ps`. | `runner/Verdict.ts` |
| S24 | **A reviewer declares its verdict; Quirks still verifies it.** `dispatchRunner` only ever emitted transport notes, so the `quote:` note the parent looked for was produced by nothing but the test suite: every real review resolved to `indeterminate` and **the default configuration could not complete a single task.** The reviewer was never even told it was reviewing — both roles got the same brief, and the "verdict" was an exit code. A reviewer's brief now carries the contract, and `runner/Verdict.ts` reads the declaration back out of the transcript. Verification is unchanged and still mechanical: declaring is how a judgment is stated, verifying is how it is believed. | `runner/Verdict.ts` |
| S25 | **A declaration line can never be its own evidence.** Found by the test written for S24: `QUIRKS-EVIDENCE: <sentence>` ends in a colon, which the quote checker counts as a statement boundary — so a reviewer could accept on any sentence it liked by typing it on the marker line, and quote verification would degrade to a spellcheck. The verification haystack now excludes declaration lines; the parser reads them from a separate view. | `runner/Quote.ts` |
| S26 | **The desktop shell loads the daemon's origin.** The packaged app served apps/web itself over a private `quirks://app` scheme, which could not work: `serviceBaseUrl()` is `""`, so `/v1/goals` resolved to index.html and the pty socket became `ws://app/`. `VITE_QUIRKS_URL` would not have fixed it either — `quirks://app` → `127.0.0.1` is cross-origin against a daemon that sends no CORS. QK-WB-009 had already made the daemon serve the workbench; loading that origin makes the shell single-origin like every other client and deletes the rival static stack (the scheme, its handler, the renderer search, the packaged `extraResources` copy). **Which ledger a packaged app opens is left open** — `QUIRKS_ROOT` else the launch directory, which is right for a dev launch and not an answer for an app in /Applications. | `DesktopEnvironment.ts` |
| S27 | **The contracts package leads the request boundary, not just the response one.** Every request-body type in `wire.ts` had sat unimported since QK-MONO-002 while the server re-derived each shape by hand in two cast kits of differing rigor — `field<T>` was a bare `as T`, so `ifRevision: "seven"` reached the ops layer typed `number`. One checked reader (`http/Body.ts`), and each POST decodes `satisfies` its contract type. `HarnessViewResponse` stops being `unknown`: the wire shape of a served route had been living in the CLI's rendering layer, invisible to the web. | `packages/contracts` |
| S28 | **The port derivation lives in `@quirks/shared`.** `Machine.ts` warns in its own header that two copies can disagree and that the failure mode is a CLI autostarting a daemon it will never find — and `apps/web/vite.config.ts` had already written a second by hand. The derivation IS the address, so it has one home that the server, the CLI, the dev proxy and the shell all import. | `packages/shared` |
