// The domain shapes the ledger persists — ported verbatim from the bun-era
// src/store/types.ts (QK-MONO-002, D3a carried). Clients import these
// type-only; the store in apps/server is the one writer.
...
/** The runners quirks can dispatch to. */
 ...
 *  durable after `--yes` (or a TTY [y/N]). */
...
export interface RunPlanEntry {
  ...
  title: string;
  order: number;
  /** Unassigned until routing exists. */
  harness: string;
  model: string;
