...
function parseArgs(argv) {
  ...
  if (!options.all) {
    throw new Error("Specify --all to uninstall across supported hosts");
  }
  return options;
}
...
async function main() {
  ...
  if (usingDefaultRoots && process.env.QUIRKS_SMOKE_APPROVED !== APPROVAL_ENV) {
    throw new Error(`Set QUIRKS_SMOKE_APPROVED=${APPROVAL_ENV} to uninstall from user host directories`);
  }
  const hosts = await uninstallAllHosts({ sourceRoot: options.source, roots });
