...
// .quirks/shape-sessions/current/; the quirks daemon serves them — no second
...
const SKILL_SCRIPTS = fileURLToPath(new URL("../../.claude/skills/shape/scripts/", import.meta.url));
...
export function ensureShapeSession(root: string, url: string): ShapePaths {
  ...
  writeFileSync(
    paths.infoFile,
    JSON.stringify({ url, screen_dir: paths.contentDir, state_dir: paths.stateDir, daemon: true }) + "\n",
  );
  return paths;
}
...
function wrapInFrame(content: string): string {
  return loadFrameTemplate().split("<!-- BRANDING -->").join(brandMarkup()).replace("<!-- CONTENT -->", content);
}
...
export function startContentWatch(root: string): void {
  ...
  for (const f of readdirSync(contentDir).filter((x) => !x.startsWith(".") && x.endsWith(".html"))) {
    try {
