...
async function createHostConfig() {
  ...
  if (!claude || !codex || !cursor) {
    return { configDir, executables: undefined, realCodexHost: codex };
  }
  await writeSmokeHostsConfig(configDir, { claude, codex, cursor });
  return { configDir, executables: { claude, codex, cursor }, realCodexHost: codex };
}

test("codex host runner smoke matrix", { skip: !APPROVED }, async () => {
  const { configDir, executables, realCodexHost } = await createHostConfig();
  if (!realCodexHost || !executables) {
    return;
  }
...
test("codex host runner smoke blocked without approval gate", { skip: APPROVED }, async () => {
  const configDir = await mkdtemp(path.join(os.tmpdir(), "quirks-smoke-codex-blocked-"));
