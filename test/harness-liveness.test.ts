...
import {
  ...
  describeLiveness,
  deriveLiveness,
} from "../src/harness/liveness.ts";
import type { Run, RunDispatchRecord } from "../src/store/types.ts";
...
describe("deriveLiveness", () => {
  test("no runs at all means never-dispatched for everyone", () => {
  ...
  test("a runner with no dispatch stays never-dispatched while a sibling answers", () => {
  ...
  test("the newest dispatch decides, across runs", () => {
    const liveness = deriveLiveness([
      run("run-001", [
  ...
  test("an older success does not override a newer failure", () => {
    const liveness = deriveLiveness([
      run("run-001", [
    ...
    expect(liveness.find((l) => l.runner === "claude")!.state).toBe("failed");
  });

  test("a timeout is its own state — silent, not broken", () => {
    const liveness = deriveLiveness([run("run-001", [dispatch({ status: "timeout", exitCode: null })])]);
    expect(liveness.find((l) => l.runner === "claude")!.state).toBe("timed-out");
  });

  test("runs with no tasks or no dispatches are tolerated", () => {
...
describe("allDispatches", () => {
  test("flattens every run and task, oldest first", () => {
...
describe("describeAge / describeLiveness", () => {
  ...
  test("an unparseable timestamp says so rather than computing nonsense", () => {
    expect(describeAge("not a date", now)).toBe("at an unparseable time");
  });

  test("a failure line carries the runner's own first line", () => {
    const liveness = deriveLiveness([
      run("run-001", [
        dispatch({
    ...
    const line = describeLiveness(liveness.find((l) => l.runner === "claude")!, now);
    expect(line).toContain("failed 60m ago");
    expect(line).toContain("usage limit reached");
    expect(line).not.toContain("second line");
  });

  test("never-dispatched reads as never dispatched, not as a failure", () => {
    const liveness = deriveLiveness([]);
    expect(describeLiveness(liveness[0]!, now)).toBe("never dispatched");
  });
});
