...
async function seedHistoricalCampaign(
  stateDir: string,
  repositoryId: string,
  campaignId: string,
): Promise<void> {
...
async function launchStandaloneWorkspace(): Promise<StandaloneFixture> {
  ...
  // Import the compiled modules so the standalone workspace serves the same built
  ...
  await seedHistoricalCampaign(stateDir, context.repositoryId, "C-hist");
  ...
  return {
    ...
    preflightUrl: `${result.authority}/preflight/C-hist${viewerFragment}`,
...
test("standalone workspace reports plan progress unavailable for a fixture task", async ({ page }) => {
  const ui = await launchStandaloneWorkspace();
  const network = attachLoopbackNetworkGuard(page, ui.authority);
  ...
  await expect(page.getByText("Campaign supervisor orchestration and crash recovery")).toHaveCount(0);
  await expectNoApprovalAffordance(page);
...
test("standalone preflight view exposes no approval affordance", async ({ page }) => {
  const ui = await launchStandaloneWorkspace();
  const network = attachLoopbackNetworkGuard(page, ui.authority);
